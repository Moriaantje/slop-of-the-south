...
| Surface photos | **ambientCG** (CC0) 1K JPG material sets, committed under `public/textures` | Roads, sidewalks, cycle paths, gravel tracks, bridge concrete, brick and plaster walls and roof tiles are photo PBR materials (colour, normal, roughness) with UVs in metres (`game/Textures.pbr`; `?pbr=0` for the flat colours). Lane markings are a separate painted layer over the asphalt. Building faces get planar UVs from their own plane so brick courses stay level. |
| Creatures and people | **Quaternius** (CC0) rigged glTF models, committed under `public/models` | Dragons, the wizard mech and the townsfolk, loaded with a vendored `GLTFLoader` (`game/Assets.js`): normalised to metres, cloned per instance with `SkeletonUtils`, one `AnimationMixer` each; a stand-in box until the file has loaded. |
| Dragons | one per lair hub (castles, ruins, the stadium, big industry) | Server-side state machines (`lib/game/dragon.rb`, 4 Hz): perch → patrol → hunt a player within 600 m → breathe fire along a 40 × 12 m strip (players on it lose 6 hp per tick unless shielded; the trees, posts and houses under it fall through `Game::Corridor`) → cooldown → home. Spells `strike` them (capped 60/45), 600 hp, dead for three minutes. The client (`game/Dragons.js`) interpolates the 4 Hz samples a quarter second behind and builds each dragon from primitives: a horned head, a 32-segment scaled body trailing the flown path, beating membrane wings. |
